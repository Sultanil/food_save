...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete') {
        ...
        $kategori = trim($_POST['kategori'] ?? 'Lainnya'); // Tambahkan ini
        ...
        try {
            // Tambahkan 'kategori' di query UPDATE
            $stmt = $pdo->prepare("UPDATE produk SET nama_produk=?, kategori=?, deskripsi=?, harga_asli=?, harga_diskon=?, stok=?, satuan=?, gambar_url=? WHERE id=? AND penjual_id=?");
            // Tambahkan $kategori di array execute
            $stmt->execute([$nama_produk, $kategori, $deskripsi, $harga_asli, $harga_diskon, $stok, $satuan, $gambar_url, $id, $penjual_id]);
        ...
        <form method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <?php if ($action === 'edit'): ?>
                ...
                <div>
                    ...
                    <select name="kategori" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 outline-none">
                        ...
                        ?>
                            <option value="<?= htmlspecialchars($kat) ?>" <?= $selected ?>>
